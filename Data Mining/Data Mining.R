...
#######Know_my_Data######
bank_data <- read.csv('35373059.csv',header = TRUE)
bank_data <- bank_data[c(3,4,6,7,1,2,5,8:19)]
######################################################
sample_rows <- sample(nrow(bank_data), nrow(bank_data)*0.80)
bank_train <- bank_data[sample_rows,]
bank_test <- bank_data[-sample_rows,]
setwd("C:/Users/User/Desktop/526")
#######Know_my_Data######
bank_data <- read.csv('35373059.csv',header = TRUE)
bank_data <- bank_data[c(3,4,6,7,1,2,5,8:19)]
bank_data <- bank_data[c(3,4,6,7,1,2,5,8:19)]
...
if (bank_data$repossess[i] == 'yes' | bank_data$bankruptcy[i] == 'yes')
{
bank_data$repos_ruptcy[i] <- 1
}
else
{
bank_data$repos_ruptcy[i] <- 0
}
}
pci <- prcomp(bank_data[,1:4],scale = TRUE)
bank_data$pc1 <- pci$x[,1]
bank_data$pc2 <- pci$x[,2]
bank_data$pc3 <- pci$x[,3]
######################################################
sample_rows <- sample(nrow(bank_data), nrow(bank_data)*0.80)
bank_train <- bank_data[sample_rows,]
bank_test <- bank_data[-sample_rows,]
y <- bank_train$default == 'no'
class(y)
y <- 1 * y
class(y)
A <- bank_train
...
A$fraud <- NULL
View(A)
A1 <- create_infotables(data=A[,-14], y="class", bins = 5)
...
A2 <- create_infotables(data=A[,-14], y="class", bins = 5)
A1 <- create_infotables(data=A[,-14], y="class", bins = 5)
train <- WOE_bin_df(A,A1)
train$class <-NULL
train$property <- NULL
View(train)
rpart.plot(tr6, type = 4, box.palette = c("green", "red"), fallen.leaves = FALSE)
tr6 <- rpart(default ~.,data = train, method = 'class', control = rpart.control(cp = 0, minsplit = 2, maxdepth = 4),parms=list(split="information"))
rpart.plot(tr6, type = 4, box.palette = c("green", "red"), fallen.leaves = FALSE)
pred <- predict(tr6, newdata = test, type = 'prob')
################
y <- bank_test$default == 'no'
class(y)
y <- 1 * y
class(y)
A <-bank_test
...
A$fraud <- NULL
A2 <- create_infotables(data=A[,-14], y="class", bins = 5)
test <- WOE_bin_df(A,A2)
test$class <- NULL
test$property <- NULL
pred <- predict(tr6, newdata = test, type = 'prob')
test$income == 0
test$income[226] <- 1
test$income[226] <- factor(test$income[226])
pred <- predict(tr6, newdata = test, type = 'prob')
rc <- roc(test$default, pred[,2],plot = TRUE, grid = TRUE)
auc(rc)
r <- c("threshold","specificity","sensitivity","accuracy","tp","tn","fp","fn")
opt <- coords(rc, x = 'best', method = 'closest.topleft', ret = r,transpose = TRUE)
opt
opt1 <- coords(rc, x = 0.25, method = 'closest.topleft', ret = r,transpose = TRUE)
opt1
####ROC############################################
train <- bank_train
View(bank_train)
...
bank_test <- bank_data[-sample_rows,]
##########filter#######
train_filtered <- bank_train
train_filtered$pc3 <- NULL
train_filtered$fraud <- NULL
train_filtered$fraud_NA <- NULL
train_filtered$property <- NULL
train_filtered$repossess <- NULL
train_filtered$bankruptcy <- NULL
train_filtered$employment <- NULL
train_filtered$l.insurance <- NULL
for (i in 1:nrow(train_filtered)){
if (train_filtered$liabilities_NA[i] == 1) {
train_filtered$any_missing[i] <- 1
} else if (train_filtered$purpose_NA[i] == 1) {
train_filtered$any_missing[i] <- 1
}   else {
train_filtered$any_missing[i] <- 0
}
}
m1 <- bestglm(train_filtered,family = binomial, IC = 'BIC')
